<?php
include "{$DOCUMENT_ROOT}/ilmea_lib.php";
...
function getJurusan($_nim){
	if (getAngkatan($_nim)<2015){
		if ((substr($_nim,1,1)==1) or (substr($_nim,1,1)==2)) {
			...
			$data = "Teknik dan Manajemen Industri Konsentrasi Teknologi Pengendalian Mutu Industri";
		}
	}else{
		if ((substr($_nim,1,1)==1) or (substr($_nim,1,1)==2)) {
			$data =  "Teknik Industri Otomotif";
		}else if ((substr($_nim,1,1)==3) or (substr($_nim,1,1)==4)) {
			$data = "Sistem Informasi Industri Otomotif";
		}else if ((substr($_nim,1,1)==5) or (substr($_nim,1,1)==6))  {
			$data =  "Teknologi Kimia Polimer";
		}else if ((substr($_nim,1,1)==7) or (substr($_nim,1,1)==8))  {
			$data =  "Administrasi Bisnis Otomotif";
		} else {
			$data = "Teknik dan Manajemen Industri Konsentrasi Teknologi Pengendalian Mutu Industri";
		}
...
function isLunas($nim,$PERIODE,$TA){
	$islunas = false;
	$totalHutang=getTotalHutang($nim,1); //hitung juga semester berjalan 
...
//	echo "0$cicilanSPP";echo "<br>";
//	echo $totalHutang;echo "<br>";
//	echo $biayaSPP;
...
function cetakHeader(){
	echo "
		<table border=0 width=650 cellpadding=0 cellspacing=0>
			<tr><td rowspan=5><img src=\"/sisformik/images/logo_stmi.jpg\" width=70 height=70>
...
function cetakTranskrip($nim){
	...
	echo "
			...
			<td align=\"center\">Jakarta, ".$tgl."<br><b>Direktur</b><br><br><br><br><br><br>
	
	
	
			  <b><u>Dr. Mustofa, ST, MT</u></b><br>
			   NIP: 19700924 200312 1 001</td></tr>
		<tr><td align=\"left\">
...
if (mysql_num_rows($theset)) {
	...
	if (!empty($detil)){	
		...
		echo "
				...
				<td align=\"center\">
					<br><br><br>
					Jakarta, ".$tgl."<br><b>Kasubag. Adm. Akademik</b><br><br><br><br><br><br>
